...
from Attenuation.AttenuationGUI import Ui_Attenuation
...
class prm():
    ...
    def GetVal(self):
        #return (self.m_val * self.x_val) + self.c_val
        return self.res_val
...
#**************************************************************************************
class AttenuationDEF(QGroupBox):
    ...
    def ConstructUI(self):
        self.instvar_names=["omega", "sstth","psw","ssw","psw_sec","ssw_sec","psp","ssp","psdiv","ssdiv","sdd"]
        self.vars_inst = {}
        for vari in self.instvar_names:
            self.vars_inst[vari] = varUI(vari,self.ui.inst_param_layout, self.scanman)
            #self.vars_inst[vari].const_lineEdit.textChanged.connect(self.UpdateUI)
        ...
        self.vars_samp_rect ={}
        self.samp_rect_names=["cor_x","cor_y","s_len_x","s_len_y","att_coef"]
        ...
        #self.ui_vars.update(self.samp_cyl_names)                       #When adding additional dics
    ...
    def Refresh(self,stth=np.linspace(0,180,50)):
        ...
        if ybuf == 0: ybuf = 1.0
        #abs_graph_axis = self.ui.abs_graphic.figure.axes[0]
        #abs_graph_axis.set_xlim(min(stth), max(stth))
        #abs_graph_axis.set_ylim(ymin - ybuf, ymax + ybuf)
        #abs_graph_axis.set_ylabel("Intensity fraction")
        #abs_graph_axis.set_title("")
        #self.absgraph.graph.figure.tight_layout()
        #self.absgraph.graph.draw()
        ...
        #cor_graph_axes.set_ylabel("Correction coefficient")
        cor_graph_axes.set_title("")
        #self.corgraph.graph.figure.tight_layout()
    ...
    def DrawLayout(self):
        ...
        #visI_r1_pts = np.array(pol2cart(ssp*cone_r1ssp_frac,cone_ang1_range)).transpose()
        #visI_r2_pts = np.array(pol2cart(ssp*1.2,cone_ang2_range)).transpose()
        #visI_pts = np.concatenate([visI_r1_pts,visI_r2_pts])
        #x=visI_pts.transpose()[0]
        #y=visI_pts.transpose()[1]
        #z1=np.random.rand(len(stth))
        ...
        if self.ui.dpattern_check.isChecked() == True:
            #if currframe.twod == True:   #2D data
            ...
            #nmax1d = max(nmax1d,currframe.y_2th.max())
        ...
        if self.ui.lock_axis_check.isChecked() == False:
            axis.autoscale(True, 'both', False)
            #axis_polar.autoscale(True, 'both', False)
