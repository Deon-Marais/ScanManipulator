...
#
# $Id: fit.py,v 1.2 2015-04-08 13:53:02 deon Exp $
# $HeadURL$
#
...
try:
    import thirdparty.pyspec.mpfit as mpfit
except ImportError:
...
__id__        = "$Id: fit.py,v 1.2 2015-04-08 13:53:02 deon Exp $"
...
##
    ...
    def _run_mpfit(self):
        ...
        if m.perror is not None:
            #DM Changed this: See the mpfit.py description when the fit is unweighted 
            #self._stdev = m.perror
            self._stdev = m.perror*sqrt(m.fnorm/m.dof)
        else:
    ...
    def _run_leastsq(self):
        ...
        plsq = np.array(leastsq(self._residuals, self._guess, Dfun = None,  full_output = 1, factor = 0.1))
        try:
            ...
            print("Error in fit: ",sys.exc_info()[0])
...
#DM            self._ilimited = self.ilimited[nonzero(self.ifix == 0)].copy()
#DM            self._ilimits = self.ilimits[nonzero(self.ifix == 0)].copy()
            self._ilimited = self.ilimited[nonzero(self.ifix == 0)[0]].copy()
            self._ilimits = self.ilimits[nonzero(self.ifix == 0)[0]].copy()
        ...
        for i in range(len(self.corr)):
            for j in range(len(self.corr)):
                ordi = sqrt(self.covar[i,i] * self.covar[j,j])
                           
                if (ordi != 0.0) :    #DM added the check for devide by zero
                    #self.corr[i,j] = self.covar[i,j] / sqrt(self.covar[i,i] * self.covar[j,j])
                    self.corr[i,j] = self.covar[i,j] / ordi
                else:
                    self.corr[i,j] = 0.0
    ...
    def chiSquared(self, norm = True, dist = 'poisson'):
        ...
        N = len(self._datax)
        P = len(self.result)
        evres = self._evalfunc()
        #self.chi2 = pow(self._evalfunc() - self._datay, 2)
        self.chi2 = pow(evres - self._datay, 2)
        if dist == 'poisson':
            evres2 = self._evalfunc()
            #self.chi2 = self.chi2 / self._evalfunc()
            if True in (evres2 == np.array([0.0]*len(evres2))):
                self.chi2 = np.array([0.0]*len(evres2))
            else:
                self.chi2 = self.chi2 / evres2
